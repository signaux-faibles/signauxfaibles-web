    ...
    <div v-if="card.userIsBoardMember">
      <v-card-title>
        ...
        <div style="margin-left: auto; margin-right: 0;" text-align="right">
          <v-btn v-if="card.url && !card.archived && kanban.visibleWekan" :href="card.url" class="px-2 ma-1"
                 color="indigo" outlined target="blank"
                 @click="trackMatomoEvent('general', 'visite_carte', card.title)">
            <v-icon class="mr-2" small>fa-link</v-icon>
             Kanban
          </v-btn>
        </div>
      </v-card-title>
      ...
      <v-card-title>
        ...
        <v-spacer/>
        <v-btn color="indigo" outlined class="px-2 ma-1"
        @click="dialogs.showCardEditor(siret, codeDepartement, denomination, card.id, card.description)"
        >
